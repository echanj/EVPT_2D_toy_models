...
note: also that random shifts are normally distributed and not uniform 
  
'''

def parallel_temper(energy_function,x0,nrep,nsteps,dim,refreq,Temp,maxdr,bs,kspr,**kwargs):
...
# initalize s
 s=np.zeros((nrep))
 s[:]=x.copy()[0,:] # this is the starting EV 

 for rep in range(nrep):
  x_new[:,rep] = local_minimization_EV(x[:,rep],s[rep],kspr[rep],energy_function)

 x = x_new
 u = energy_function(x,s,kspr,**kwargs) # initial unbiased potential energy of the particle
 xTraj = np.zeros((dim,nrep,bs,nsteps))
 sTraj = np.zeros((nrep,nsteps))
 ...
 for step in range(nsteps):
        ...
        if (np.random.rand() < refreq):
          ...
          # pick a random replica - in EVRE we are only swapping on the EV   
            ...
            if (deltaBeta*deltaU > 0.0):
                ...
                # Exchange s positions and energies  
                s[[rxch0,rxch1]]=s[[rxch1,rxch0]]    # this is syntactical numpy and will be handy when the number of reps is arbitary 
        ...
        for rep in range(nrep):
            ...
            # sTrial = s[rep] + maxdr[rep]*np.random.randn(*s[rep].shape)
            sTrial = s[rep] + maxdr[rep]*2*np.random.rand(*s[rep].shape)-0.5
            # print(sTrial)
            # perform local optimization

 ########## this is where is make an inner lop treatment for the minibatch  

            
            xTrial=np.zeros((dim,bs)) # our trials for this replica in this step
            xTrial[0,:]=xTrial[0,:]+sTrial # the x values are = s
            xTrial[1,:]=4*np.random.rand(bs)-2.0  # the y values are chosen using psudeo random 
                                                  # or quasi random 
                                                  # the range is the chosen extent of the y-var
                                                  # movements along this axis appear akin to being temperature accelerated 
                                                    

            # print (xTrial)
            for m in range(bs):
             # print (xTrial[:,m])
             xTrial[:,m] = local_minimization_EV(xTrial[:,m],sTrial,kspr[rep],energy_function)
             # print (xTrial[:,m])
            
       
            # Calculate the potential energy at the proposed position
            uTrial = energy_function(xTrial,sTrial,kspr[rep],**kwargs)

            umin=np.min(uTrial)
            idx_min=np.where(uTrial==umin)
            ...
            # Save the positions in the trajectory and the bath ID
            sTraj[rep,step]=s[rep]
           # print("Temp: %.2f  s: %.3f  xmin:  %.3f  ymin:  %.3f "%(Temp[rep],s[rep],x[0,rep],x[1,rep]) )
            for m in range(bs):
             xTraj[:,rep,m,step] = xTrial[:,m]

        iter_to_screen=20
        alpha=0.5 # 1.0 # learning rate
        rstep=step+1  # adjusted step for determining ratios
        if ((step%int(np.round(nsteps/iter_to_screen)) == 0) and (rstep != 0) ) : 
         for rep in range(nrep):
            print("step: {:d} Temp: {:.3f} acc1: {:.3f} acc2: {:.3f} rej: {:.3f} maxdr: {:.3f} dev: {:.3f}  kspr: {:.3f} ".format(step, Temp[rep], 
                                                      acc1[rep]/rstep, acc2[rep]/rstep, rej[rep]/rstep, maxdr[rep], -alpha*maxdr[rep]*((rej[rep]/rstep) - 0.5), kspr[rep] ))
        update_maxdr=False
        update_freq=10
        if ((step%update_freq == 0) and (step != 0) and update_maxdr ) : 
           for rep in range(nrep):
          ...
          # we want to optimize maxdr given the constraint rej=0.5
             maxdr[rep] -= ((rej[rep]/rstep) - 0.5)*alpha*abs(maxdr[rep]) 
            #  kspr[rep] -= ((rej[rep]/rstep) - 0.5)*alpha*abs(kspr[rep]) 


 for rep in range(nrep):
  print("step: {:d} Temp: {:.3f} acc1: {:.3f} acc2: {:.3f} rej: {:.3f}".format(step, Temp[rep], acc1[rep]/rstep, acc2[rep]/rstep, rej[rep]/rstep))
  print("exchange rates: ex1c: {:.3f}  ex2c: {:.3f} ".format(ex1c/rstep,ex2c/rstep))
  print("final stepsizes: ")
  print(maxdr)
  print("final springcon: ")
  print(kspr)
