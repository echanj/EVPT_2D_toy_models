...
''' 
in this implementation the dimensionallity of enegy surface must be 1D
The other implementation will support 2D and higher pes.

'''

def parallel_temper(energy_function,x0,nrep,nsteps,refreq,Temp,maxdr,**kwargs):
...
# set up an empty trajectory
 ...
 for step in range(nsteps):
        ...
        for rep in range(nrep):
            ...
            xTrial = x[rep] + maxdr[rep]*(np.random.rand()-0.5)
        ...
        if ((step%2 == 0) and (step != 0)) : 
          ...
          # below is similar to a finite difference method
              maxdr[rep] -=   ((rej[rep]/step) - 0.5)*alpha*maxdr[rep]   
          #    maxdr[rep] -=  0.02 
          #  else  :
          #    maxdr[rep] += 0.02
...
# Set simulation parameters
 ...
 xTraj = parallel_temper(dw,x,nrep,nsteps,refreq,Temp,maxdr)
