import numpy as np 
from optimize_functions import local_minimization
...
# set up an empty trajectory
 ...
 for rep in range(nrep):
  x_new[:,rep] = local_minimization(x[:,rep],energy_function)
 ...
 for step in range(nsteps):
        ...
        for rep in range(nrep):
            ...
            xTrial = x[:,rep] + 2.0*maxdr[rep]*(np.random.rand(dim)-0.5)
            # xTrial = x[:,rep] + maxdr[rep]*np.random.randn(*x[:,rep].shape)
            # perform local optimization

            xTrial = local_minimization(xTrial,energy_function)
        ...
        if ((step%int(np.round(nsteps/iter_to_screen)) == 0) and (rstep != 0) ) : 
         for rep in range(nrep):
            print("step: {:d} Temp: {:.3f} acc1: {:.3f} acc2: {:.3f} rej: {:.3f} maxdr: {:.3f} dev: {:.3f}".format(step, Temp[rep], 
                                                      acc1[rep]/rstep, acc2[rep]/rstep, rej[rep]/rstep, maxdr[rep], -alpha*maxdr[rep]*((rej[rep]/rstep) - 0.5) ))
        update_maxdr=True
        update_freq=10
